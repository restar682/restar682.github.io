...
子类型（Subtyping）关系用于描述类型之间的继承与兼容性，记作 $$T_1 \leq T_2$$，表示类型 $$T_1$$ 是 $$T_2$$ 的子类型。其基本性质包括：
...
$$
\text{if } e_0 \text{ then } e_1 \text{ else } e_2 \text{ fi}
$$
的类型为
$$
lub(T_1, T_2)
$$ 
即 $e_1$ 和 $e_2$ 类型的**最小上界**，且 $e_0$ 必须为 $Bool$ 类型。

#### 3. case 表达式的类型

$$
\text{case } e_0 \text{ of } \ldots \text{ esac}
$$
的类型为
$$
lub(T_1', T_2', \ldots, T_n')
$$ 
即所有分支结果类型的**最小上界**。
...
在 Cool 中，对象标识符（变量）和方法标识符（函数名）属于**不同的命名空间**。  
这意味着在同一个作用域里，可以同时存在一个对象 $$foo$$ 和一个方法 $$foo$$，它们互不冲突。

为了支持方法的类型检查，类型系统中需要一个额外的**方法环境** $$M$$，专门存储方法的签名信息。
...
在涉及 $$SELF\_TYPE$$ 的场景下，仅凭 $$O$$（对象环境）和 $$M$$（方法环境）还不够。  
因为 $$SELF\_TYPE$$ 的含义依赖于方法所在的类，所以还需要引入 **当前类 $$C$$** 作为上下文。

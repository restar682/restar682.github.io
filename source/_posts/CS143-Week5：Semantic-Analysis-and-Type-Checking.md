...
`if e₀ then e₁ else e₂ fi` 的类型为 `lub(T₁, T₂)`，即 `e₁` 和 `e₂` 类型的**最小上界**，且 `e₀` 必须为 `Bool` 类型。

#### 3. `case` 表达式的类型

`case e₀ of ... esac` 的类型为 `lub(T₁', T₂', ..., Tₙ')`，即所有分支结果类型的**最小上界**。
...
在 Cool 中，对象标识符（变量）和方法标识符（函数名）属于**不同的命名空间**。这意味着在同一个作用域里，可以同时存在一个对象 $$foo$$ 和一个方法 $$foo$$，它们互不冲突。

...
## 算术表达式的代码生成
算术表达式的代码生成就是将表达式的结构直接映射为指令的执行序列。由于栈机依赖隐式操作数栈，计算一个算术表达式只需依次生成其子表达式的求值代码，再追加对应的操作指令。例如，表达式 `e₁ + e₂` 的代码并非独立存在，而是由 `e₁` 的代码、`e₂` 的代码和一条 `add` 指令拼接而成——这本质上是一个**递归构造过程**。我们可以把每一个运算符看作一个“模板”，用其操作数的代码递归填充“空位”，不难想到利用 AST 递归下降遍历即可：遇到叶子节点（如常量或变量）时生成 `push` 指令；遇到内部节点（如二元运算）时，先递归生成左右子树的代码，再输出操作指令。

在这个过程中，只需要用到三个寄存器：$a_0,t_1,sp$，查表可以发现他们都是 Caller-Saved 寄存器

需要注意的是：在栈机的规范中，要求在完成整个算术表达式的代码生成后，栈指针应保持不变。除此之外，不能依赖寄存器的值，因为它们属于 Caller-Saved 寄存器，调用者必须自行负责保存与恢复。

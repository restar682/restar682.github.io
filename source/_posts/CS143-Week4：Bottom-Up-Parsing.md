...
一个重要事实是：**句柄总是位于分析栈的顶部，而不会隐藏在栈的中间或底部**。这是因为在语法分析的推进过程中，所有可以归约的左侧结构都已经被逐步归约完毕，而尚未展开的非终结符无法参与当前归约。因此，第一个可被完整归约的语法单位——即句柄——必然出现在栈顶。
...
> 递归下降解析就像普通的递归版线段树，直观自然但依赖系统调用栈，控制不够灵活；而自底向上解析则类似于 zkw 线段树，利用文法的结构性质（如最右推导、句柄总在栈顶），将原本隐式的递归过程转化为显式的迭代操作，通过手动维护分析栈和查表驱动的方式，把复杂的递归逻辑“展平”，不仅避免了函数调用开销，还提升了效率与表达能力。所以，当问题具有良好的结构规律时，我们或许总能够将“自然递归”转化为“高效迭代”，用数据结构模拟控制流，实现性能与简洁性的统一。
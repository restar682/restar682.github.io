...
自底向上解析是语法推导的逆过程，具体对应最右推导的反向执行，即归约顺序与最右推导步骤相反。

在解析过程中，最关键的是要找到当前可以归约的部分，这个部分叫做“句柄”。句柄决定了下一步应该应用哪条语法规则。一个重要事实是：句柄总是出现在分析栈的顶部，不会藏在中间或底部。这意味着解析器只需要看栈顶的内容，就能决定是否可以归约，不需要来回查找或回溯。这使得解析过程可以高效进行。

还有一个重要的理论基础是：在分析过程中可能出现的所有合法前缀（称为可行前缀），它们的集合是一个正则语言。这意味着我们可以用一个有限状态自动机来识别这些前缀。这个性质使得我们可以预先构造一个状态机来指导解析过程，从而实现高效、确定的分析。

在实现上，自底向上解析通常使用一个栈来保存已经处理的符号和状态，再配合一张分析表来决定每一步该做什么。分析表告诉解析器：在某个状态下，看到某个输入符号时，应该移进、归约、接受还是报错。整个过程是自动推进的，不需要回溯。

相比自顶向下的方法，自底向上解析更强大。它可以处理左递归文法，不需要对文法做额外的变换，比如消除左递归或提取左因子。这使得它可以更自然地描述编程语言中的常见结构，比如连续的加减乘除表达式。

正因为这些优点，自底向上解析成为工业级编译器的主流选择。许多语法生成工具，比如 Yacc 和 Bison，都是基于这种思想实现的。
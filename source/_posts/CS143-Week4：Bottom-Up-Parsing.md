...
还有一个重要的性质是：在分析过程中可能出现的所有合法前缀构成的集合是一个正则语言。进一步地，每一个合法前缀都唯一对应一组可能的语法位置，我们用 **LR 项**（如 `E → .(T)`、`E → (.T)`）来表示这些位置，其中“点”表示当前解析的进度。
...
于是，我们可以构造一个**有限状态自动机**，每个状态是一个项集，表示当前可能所处的所有语法位置；每条边表示一个“移进”或“归约”动作，驱动点在产生式中移动。以文法 $ E \to (T) $ 为例，状态转移如下：

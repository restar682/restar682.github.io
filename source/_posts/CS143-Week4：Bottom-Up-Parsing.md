...
一个重要事实是：**句柄总是位于分析栈的顶部，而不会隐藏在栈的中间或底部**。或者说，句柄的右端位置是不减的。这是因为自底向上分析对应的是最右推导的逆过程，若句柄的右端位置回退，将与最右推导的顺序相冲突。但这并不意味着一旦在栈顶出现了可归约的结构就需要归约，当产生式有公共右因子，贸然归约可能导致错误。

还有一个重要的性质是：在分析过程中可能出现的所有合法前缀构成的集合是一个正则语言。这意味着我们可以用一个有限状态自动机来识别这些前缀。这个性质使得我们可以预先构造一个状态机来指导解析过程，将复杂度从 $O(n^2)$ 变为 $O(n)$，大大提升了效率。
...
但幸运的是，存在一系列有效的启发式方法来“猜测”当前栈顶是否为句柄。这些方法基于语法规则的局部特征和输入符号进行判断，能够在许多实际场景中快速做出正确决策。更进一步，对于一大类精心设计的上下文无关文法（如 LL、LR 文法），这些启发式策略总是能准确识别句柄。

因此，LR 解析器能够通过预先构造状态机和分析表，将句柄识别转化为 `O(1)` 的查表操作，从而在实践中实现高效、确定的线性时间解析。因此，虽然通用句柄识别是困难的，但在实际编程语言的文法约束下，我们依然能够构建出强大而高效的解析器。

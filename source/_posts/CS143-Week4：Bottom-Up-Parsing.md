...
一个重要事实是：**句柄总是位于分析栈的顶部，而不会隐藏在栈的中间或底部**。这是因为在语法分析的推进过程中，所有可以归约的左侧结构都已经被逐步归约完毕，因此，第一个可被完整归约的语法单位——即句柄——必然出现在栈顶。但这并不意味着一旦在栈顶出现了可归约的结构就需要归约，当产生式有公共右因子，贸然归约可能导致错误。
...
- **常见例子**：
  - 两个不同非终结符有相同的右部：
    ...
    FuncCall → id '(' ArgList ')'
    ```
    当解析器看到 `id(...)` 时，无法判断是归约为 `Stmt` 还是 `FuncCall`。
  - 类型系统或作用域相关的歧义文法。

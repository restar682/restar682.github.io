...
由于我们拥有无限的寄存器，生成中间代码的过程会变得非常简单。我们无需像生成 MIPS 指令那样用不止一条指令来推送结果、调整栈指针以及各式各样繁琐的操作，只需要将后续要用到的结果保存在一个寄存器中，之后需要再直接取出即可。
...
在实践中，编译器开发者通常会有意放弃实现某些已知的“高级”优化，原因往往在于实现复杂度高、编译开销大、实际收益有限等等。而许多所谓的“炫技式”优化，甚至同时具备这三个缺点，自然不被采用。因此，编译器优化的核心原则是：  
> **以最小的代价，获取最大的收益**。

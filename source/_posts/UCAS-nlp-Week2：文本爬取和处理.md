...
由于 `Counter` 是 `dict` 的子类，它直接继承了字典的通用接口，支持以下常用操作：

- `c[key]`：获取 `key` 对应的计数值；若 `key` 不存在，返回 `0`
- `c.keys()`：返回一个包含所有元素（键）的字典视图  
- `c.values()`：返回一个包含所有计数值（值）的字典视图  
- `c.items()`：返回一个包含 `(key, count)` 键值对的字典视图，适用于迭代或转换为列表  

`Counter` 仅提供一种查询与排序方法：
- `c.most_common(n=None)`：返回出现次数最多的前 `n` 个元素及其计数，按频率降序排列；若 `n=None`（默认），则返回所有元素  

`Counter` 支持改变一些元素的计数：
- `c.update(iterable-or-mapping)`：对现有计数进行增量累加  
- `c.subtract(iterable-or-mapping)`：从现有计数中减去指定值，允许结果为负  

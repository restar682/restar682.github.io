...
categories: CS143
description: 语法分析的目标、文法基础与递归下降解析技术。
...
- **鲁棒地处理语法错误：** 当输入序列存在语法错误时，能够优雅地进行错误恢复，尽可能地继续分析后续代码，报告有意义的错误信息，并尝试构建部分正确的语法树，以提升编译器的用户体验和诊断能力。

尽管在实现上存在将词法分析与语法分析合并的编译器，大多数编译器仍然将二者分离。这是因为词法分析通常涉及正则语言，适合用正则表达式或有限自动机处理，而语法分析涉及到上下文无关结构，单独处理更为清晰。
...
左递归问题是可解的，并且有成熟的算法可以**自动消除**直接和间接左递归。然而在实践中，开发者更倾向于通过引入新的非终结符和右递归规则来**手动消除**左递归。例如，将直接左递归规则：

$$A\rightarrow A\ \alpha_1 | A\ \alpha_2 | \cdots | A\ \alpha_m| \beta_1 | \beta_2 | \cdots | \beta_n$$

引入新的语法变量，转换为等价的右递归形式：

$$A \rightarrow \beta_1 A' | \beta_2 A'| \cdots | \beta_n A'$$
$$A' \rightarrow \alpha_1 A' | \alpha_2 A' | \cdots | \alpha_m A' | \varepsilon$$

对于间接左递归规则，我们先人为将变量全部重命名为 $A_1,A_2,\cdots,A_m$，然后我们从 $i=1,2,\cdots,m$ 依次处理，使得处理后产生式要么以终结符开头，要么以 $A_k$ 开头，其中 $k>i$。

具体来说，对 $j=1,2,\cdots,i-1$，其已经处理过，设所有以 $A_j$ 为左部的产生式为 $A_j\rightarrow \delta_1 | \delta_2 | \cdots | \delta_t$，那么将以 $A_i$ 为左部以 $A_j$ 为右部开头的产生式 $A_i\rightarrow A_j \lambda$ 替换为 $A_i\rightarrow \delta_1 \lambda| \delta_2 \lambda| \cdots | \delta_t\lambda$ 后，再消除 $A_i$ 的直接左递归，$i$ 也满足条件，如此处理后就消除了所有的左递归。

之所以常采用手动方式解决左递归问题，一方面是因为对于常见的语法结构（如表达式），手动转换模式简单且易于理解；另一方面，手动转换灵活许多，可以根据实际需求定制解决方案。因此，解决左递归问题一般都采用手动消除的方法。
